...
class PrioritizationService:
    ...
    _FOCUS_REASONS = {
        FocusArea.MIND_MAP: "先把核心概念和关系理清，后续写作会更稳定。",
        FocusArea.SUMMARY: "当前已经积累了若干要点，适合先压缩成可继续修改的概要。",
        FocusArea.PRACTICE_PROBLEM: "行动研究首先要落在真实、可观察的实践困境上。",
        FocusArea.LITERATURE_EVIDENCE: "把问题和前人研究连起来，论证会更扎实。",
        FocusArea.RESEARCH_PROBLEM: "先把研究问题说准，后续目标、干预和评估才容易对齐。",
        FocusArea.EXPECTED_OUTCOME: "需要先明确希望看到什么变化，才能判断行动是否有效。",
        FocusArea.INTERVENTION_PLAN: "问题和目标初步明确后，应尽快落到可执行的教学行动。",
        FocusArea.DATA_COLLECTION_AND_REFLECTION: "行动之后要想形成研究闭环，就需要想清楚证据和反思方式。",
    }

    _FOCUS_GUIDANCE = {
        FocusArea.MIND_MAP: "可以先把研究对象、课堂现象、可能原因、干预策略和预期变化列成几个节点，再画出它们之间的关系。",
        FocusArea.SUMMARY: "可以先写一个短版本摘要，包含问题、目标、行动和预期证据，后面再逐步精炼语言。",
        FocusArea.PRACTICE_PROBLEM: "建议先把问题描述得具体、可观察，尽量落到某类课堂表现，而不是停留在宽泛判断上。",
        FocusArea.LITERATURE_EVIDENCE: "可以先找两三类最相关的理论或研究方向，用来支撑你对问题成因和干预价值的判断。",
        FocusArea.RESEARCH_PROBLEM: "建议先把研究问题收窄到一个明确场景、一个核心对象和一种可观察变化，便于后续设计行动。",
        FocusArea.EXPECTED_OUTCOME: "可以先区分短期变化和可观察指标，例如参与人数、回答质量、任务完成度等。",
        FocusArea.INTERVENTION_PLAN: "建议先把干预写成可执行步骤，包括对象、频率、周期和课堂中的具体做法。",
        FocusArea.DATA_COLLECTION_AND_REFLECTION: "可以先确定两三种最容易实施的证据来源，再配一个反思问题来帮助进入下一轮修订。",
    }
    ...
    _KEYWORDS = {
        FocusArea.MIND_MAP: ("思维导图", "脑图", "mind map", "map"),
        ...
        FocusArea.RESEARCH_PROBLEM: ("研究问题", "研究主题", "我想研究", "问题是什么"),
